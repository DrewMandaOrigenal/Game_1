...
class Bat:
    #constructor
    def __init__ (self, table, width=15, height=100, x_posn=50, colour="green", x_speed=23, y_speed=23):
        self.width = width
        self.height = height
        self.x_posn = x_posn
        self.y_posn = y_posn
    ...
    def move_right(self, master):
        ...
        y1 = self.y_posn
        y2 = self.y_posn+self.height
        seld.table.move_item(self.rectangle, x1, y1, x2, y2)

    def start_position(self):
        self.x_posn = self.x_start
        seld.y_posn = self.y_start
    ...
    def detect_collision(self, ball, sides_sweet_spot=True, topnbottom_sweet_spot=False):
        ...
        if(collision):
            ...
            elif((left_b > left-r) and (right_b < right) and (bottom_b > right) and (left_b <= bottom)):
